...
public static class ScoreSaberPpCalculator
{
    private const double BasePpMultiplier = 42.117208413d;

    private static readonly (double Accuracy, double Multiplier)[] Curve =
    {
    ...
    public static double Calculate(double stars, double accuracy, bool failed = false)
    {
        if (stars <= 0d)
        {
            return 0d;
        ...
        var effectiveAccuracy = Clamp01(accuracy);
        if (failed)
        {
            effectiveAccuracy *= 0.5d;
        }

        return Math.Max(0d, MultiplierAt(effectiveAccuracy) * stars * BasePpMultiplier);
    }

    private static double MultiplierAt(double accuracy)
    {
        ...
        for (var i = 0; i < Curve.Length - 1; i++)
        {
            var high = Curve[i];
            var low = Curve[i + 1];
            if (Math.Abs(high.Accuracy - clamped) < 0.0000001d)
            {
                return high.Multiplier;
            }

            if (low.Accuracy < clamped)
            {
                var slope = (high.Multiplier - low.Multiplier) / (high.Accuracy - low.Accuracy);
                var intercept = low.Multiplier - (slope * low.Accuracy);
